...
if ($_SESSION['role'] !== 'admin') {
    die('Access Denied: You do not have permission to update orders.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($form_data['id']) || empty($form_data['nama_customer']) || empty($form_data['kode_pisau']) || empty($form_data['jenis_board']) || empty($form_data['cover_dalam_supplier']) || empty($form_data['cover_dalam_jenis']) || empty($form_data['cover_dalam_warna']) || empty($form_data['cover_dalam_gsm']) || empty($form_data['cover_dalam_ukuran']) || empty($form_data['cover_luar_supplier']) || empty($form_data['cover_luar_jenis']) || empty($form_data['cover_luar_warna']) || empty($form_data['cover_luar_gsm']) || empty($form_data['cover_luar_ukuran']) || empty($form_data['box_supplier']) || empty($form_data['box_jenis']) || empty($form_data['box_warna']) || empty($form_data['box_gsm']) || empty($form_data['box_ukuran']) || empty($form_data['dudukan_supplier']) || empty($form_data['dudukan_jenis']) || empty($form_data['dudukan_warna']) || empty($form_data['dudukan_gsm']) || empty($form_data['dudukan_ukuran'])) {
        die('Error: Order ID, Nama Customer, Kode Pisau, Jenis Board, and all Cover Dalam, Cover Luar, Box, and Dudukan fields are required.');
    }
    ...
    if ($kode_pisau === 'baru') {
        if (empty($form_data['model_box_baru']) || empty($form_data['length']) || empty($form_data['width']) || empty($form_data['height'])) {
        ...
        // Update barang table if necessary, or just use the values
        // For now, we'll just use the values directly in the orders table update

    } else { // lama
        if (empty($form_data['barang_lama'])) {
    ...
    // Shipping data (these fields are not in edit_fo.php, so they will be null or empty if not passed)
    // For now, we will assume these are not being updated via edit_fo.php directly.
    // If they need to be updated, they should be added to edit_fo.php
    $tanggal_kirim = $form_data['tanggal_kirim'] ?? NULL;
    $jam_kirim = $form_data['jam_kirim'] ?? NULL;
    $dikirim_dari = $form_data['dikirim_dari'] ?? NULL;
    $tujuan_kirim = $form_data['tujuan_kirim'] ?? NULL;
