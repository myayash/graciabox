...
if (isset($_GET['archive_id']) && !empty($_GET['archive_id'])) {
    if (!$is_admin) { // Only allow admin to archive
        header("Location: " . BASE_URL . "/daftar_spk_logo");
        exit;
    }
    ...
    try {
        ...
        header("Location: " . BASE_URL . "/daftar_spk_logo"); // Redirect to refresh the page
        exit;
...
if (isset($_GET['unarchive_id']) && !empty($_GET['unarchive_id'])) {
    ...
    try {
        ...
        header("Location: " . BASE_URL . "/daftar_spk_logo?show_archived=true"); // Redirect to refresh the page, staying on archived view
        exit;
    } catch (PDOException $e) {
...
<h1 class="text-xl sm:text-2xl font-bold mb-6 text-gray-800 flex flex-col sm:flex-row sm:items-center">
    ...
    <div class="flex flex-col sm:flex-row sm:inline-flex mt-2 sm:mt-0 space-y-2 sm:space-y-0 sm:space-x-2">
        <?php if (isset($_GET['show_archived']) && $_GET['show_archived'] == 'true'): ?>
            <a href="daftar_spk_logo" class="px-4 py-2 bg-gray-600 text-white font-bold hover:bg-gray-700 transition duration-150 ease-in-out rounded-md">Active SPK Logo</a>
        <?php else: ?>
            <a href="daftar_spk_logo?show_archived=true" class="sm:ml-2 px-4 py-2 bg-gray-600 text-white font-bold hover:bg-gray-700 transition duration-150 ease-in-out rounded-md">Archived SPK Logo</a>
        <?php endif; ?>
    </div>
    <a href="daftar_spk" class="sm:ml-auto mt-2 sm:mt-0 px-4 py-2 bg-blue-600 text-white font-bold hover:bg-blue-700 transition duration-150 ease-in-out rounded-md">SPK Dudukan</a>
    <?php endif; ?>
</h1>

<form action="daftar_spk_logo" method="get" class="mb-4 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
    <input type="text" name="search" placeholder="Cari data SPK Logo" value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>" class="p-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-md w-full sm:w-auto">
...
$sql = "SELECT id, nama, ukuran, quantity, dibuat, model_box, logo, ukuran_poly, lokasi_poly, klise, logo_img, poly_img, is_archived FROM spk_logo";
    ...
    if (isset($_GET['search']) && $_GET['search'] !== '') {
        ...
        $searchable_columns = ['id', 'nama', 'ukuran', 'quantity', 'dibuat', 'model_box', 'logo', 'ukuran_poly', 'lokasi_poly', 'klise'];
    ...
    if ($spk_list) {
        ...
        foreach ($spk_list as $spk) {
            ...
            if ($is_admin) {
                ...
                echo "<a href=\"view_spk_logo_pdf?id=" . htmlspecialchars($spk['id']) . "\" class=\"text-blue-600 hover:text-blue-900 mr-2\">View</a>";
                if (isset($_GET['show_archived']) && $_GET['show_archived'] == 'true') {
                    echo "<a href=\"daftar_spk_logo?unarchive_id=" . htmlspecialchars($spk['id']) . "\" onclick=\"return confirm('Are you sure you want to unarchive this SPK?');\" class=\"text-green-600 hover:text-green-900\">Unarchive</a>";
                } else {
                    echo "<a href=\"daftar_spk_logo?archive_id=" . htmlspecialchars($spk['id']) . "\" onclick=\"return confirm('Are you sure you want to archive this SPK?');\" class=\"text-red-600 hover:text-red-900\">Archive</a>";
                }
                echo "</td>";
            }
            echo "</tr>";
