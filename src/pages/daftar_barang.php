...
if (isset($_GET['archive_id']) && !empty($_GET['archive_id'])) {
    ...
    try {
        ...
        header("Location: " . BASE_URL . "/daftar_barang"); // Redirect to refresh the page
        exit;
...
if (isset($_GET['unarchive_id']) && !empty($_GET['unarchive_id'])) {
    ...
    try {
        ...
        header("Location: " . BASE_URL . "/daftar_barang?show_archived=true"); // Redirect to refresh the page, staying on archived view
        exit;
    } catch (PDOException $e) {
...
<h1 class="text-xl sm:text-2xl font-bold mb-6 text-gray-800 flex flex-col sm:flex-row sm:items-center">
    <span class="mr-4">daftar barang</span>
    <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
    <div class="flex flex-col sm:flex-row sm:inline-flex mt-2 sm:mt-0 sm:ml-4 space-y-2 sm:space-y-0 sm:space-x-2">
        <a href="<?php echo BASE_URL; ?>/bikin_barang" class="px-4 py-2 bg-blue-600 text-white font-bold hover:bg-blue-700 transition duration-150 ease-in-out rounded-md">+ Barang</a>
        <?php if (isset($_GET['show_archived']) && $_GET['show_archived'] == 'true'): ?>
            <a href="<?php echo BASE_URL; ?>/daftar_barang" class="sm:ml-2 px-4 py-2 bg-gray-600 text-white font-bold hover:bg-gray-700 transition duration-150 ease-in-out rounded-md">Active Barang</a>
        <?php else: ?>
            <a href="<?php echo BASE_URL; ?>/daftar_barang?show_archived=true" class="sm:ml-2 px-4 py-2 bg-gray-600 text-white font-bold hover:bg-gray-700 transition duration-150 ease-in-out rounded-md">Archived Barang</a>
        <?php endif; ?>
    </div>
    <?php endif; ?>
</h1>

<form action="<?php echo BASE_URL; ?>/daftar_barang" method="get" class="mb-4 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
    <input type="text" name="search" placeholder="cari data barang" value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>" class="p-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-md w-full sm:w-auto">
...
try {
    ...
    if (isset($_GET['search']) && $_GET['search'] !== '') {
        ...
        $searchable_columns = ['id', 'model_box', 'ukuran', 'nama', 'dibuat'];
    ...
    if ($barangs) {
        ...
        foreach ($barangs as $barang) {
            ...
            if ($is_admin) {
                ...
                echo "<a href=\"" . BASE_URL . "/edit_barang?id=". htmlspecialchars($barang['id']) . "\" class=\"text-indigo-600 hover:text-indigo-900\">Edit</a>";
                if (isset($_GET['show_archived']) && $_GET['show_archived'] == 'true') {
                    echo "<a href=\"" . BASE_URL . "/daftar_barang?unarchive_id=". htmlspecialchars($barang['id']) . "\" onclick=\"return confirm('Are you sure you want to unarchive this barang?');\" class=\"text-green-600 hover:text-green-900\">Unarchive</a>";
                } else {
                    echo "<a href=\"" . BASE_URL . "/daftar_barang?archive_id=". htmlspecialchars($barang['id']) . "\" onclick=\"return confirm('Are you sure you want to archive this barang?');\" class=\"text-red-600 hover:text-red-900\">Archive</a>";
                }
                echo "</td>";
            }
            echo "</tr>";
        ...
        echo "<p class=\"mt-4 text-gray-600\">No barang found in the database. Click +Barang to create a new one.</p>";
    }
} catch (PDOException $e) {
