...
if ($_SESSION['role'] !== 'admin') {
    die('Access Denied: You do not have permission to add new aksesoris.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($jenis) && !empty($ukuran) && !empty($warna)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO aksesoris (jenis, ukuran, warna) VALUES (?, ?, ?)");
            $stmt->execute([$jenis, $ukuran, $warna]);
            header("Location: " . BASE_URL . "/daftar_aksesoris");
            exit;
        } catch (PDOException $e) {
            echo "<p class=\"mt-4 text-red-600\">Error adding aksesoris: " . htmlspecialchars($e->getMessage()) . "</p>";
        }
    } else {
        echo "<p class=\"mt-4 text-red-600\">Please fill in all required fields (Jenis, Ukuran, and Warna).</p>";
    }
}
?>
...
<body class="bg-gray-100 text-gray-900 pt-24 px-8 pb-8">
    ...
    <h1 class="text-xl sm:text-2xl font-bold mb-6 text-gray-800">tambah aksesoris</h1>

    <form action="<?php echo BASE_URL; ?>/bikin_aksesoris" method="post" class="bg-white p-8 shadow-md rounded-lg max-w-md mx-auto">
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="ukuran" id="ukuran" class="appearance-none bg-white border border-gray-300 w-full py-2 px-3 text-gray-800 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out rounded-md" required>
        </div>
        ...
        <div class="flex items-center justify-start space-x-4">
            ...
            <a href="<?php echo BASE_URL; ?>/daftar_aksesoris" class="inline-block align-baseline font-semibold text-sm text-blue-600 hover:text-blue-800">
                Cancel
            </a>
        </div>
    </form>
