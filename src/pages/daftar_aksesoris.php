...
<h1 class="text-xl sm:text-2xl font-bold mb-6 text-gray-800 flex flex-col sm:flex-row sm:items-center">
    <span class="mr-4">daftar aksesoris</span>
    <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
    <div class="flex flex-col sm:flex-row sm:inline-flex mt-2 sm:mt-0 sm:ml-4 space-y-2 sm:space-y-0 sm:space-x-2">
        <a href="bikin_aksesoris" class="px-4 py-2 bg-blue-600 text-white font-bold hover:bg-blue-700 transition duration-150 ease-in-out rounded-md">+ Aksesoris</a>
        <?php if (isset($_GET['show_archived']) && $_GET['show_archived'] == 'true'): ?>
            <a href="daftar_aksesoris" class="sm:ml-2 px-4 py-2 bg-gray-600 text-white font-bold hover:bg-gray-700 transition duration-150 ease-in-out rounded-md">Active Aksesoris</a>
        <?php else: ?>
            <a href="daftar_aksesoris?show_archived=true" class="sm:ml-2 px-4 py-2 bg-gray-600 text-white font-bold hover:bg-gray-700 transition duration-150 ease-in-out rounded-md">Archived Aksesoris</a>
        <?php endif; ?>
    </div>
    <?php endif; ?>
</h1>

<form action="daftar_aksesoris" method="get" class="mb-4 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
    <input type="text" name="search" placeholder="cari data aksesoris" value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>" class="p-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-md w-full sm:w-auto">
...
<?php
try {
    $sql = "SELECT id, jenis, ukuran, warna, is_archived FROM aksesoris";
    $conditions = [];
    $params = [];
    ...
    if (isset($_GET['search']) && $_GET['search'] !== '') {
        ...
        $searchable_columns = ['id', 'jenis', 'ukuran', 'warna'];
    ...
    $stmt->execute($params);
    $aksesoris_data = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    if ($aksesoris_data) {
        ...
        foreach (array_keys($aksesoris_data[0]) as $columnName) {
        ...
        foreach ($aksesoris_data as $aksesoris) {
            echo "<tr>";
            foreach ($aksesoris as $columnName => $value) {
                if ($columnName == 'is_archived') continue;
