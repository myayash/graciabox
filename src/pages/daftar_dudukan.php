...
if (isset($_GET['archive_id']) && !empty($_GET['archive_id'])) {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE dudukan SET is_archived = 1 WHERE id = ?");
        $stmt->execute([$archive_id]);
        header("Location: " . BASE_URL . "/daftar_dudukan"); // Redirect to refresh the page
        exit;
    } catch (PDOException $e) {
        echo "<p>Error archiving dudukan: " . htmlspecialchars($e->getMessage()) . "</p>";
    }
}
...
if (isset($_GET['unarchive_id']) && !empty($_GET['unarchive_id'])) {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE dudukan SET is_archived = 0 WHERE id = ?");
        $stmt->execute([$unarchive_id]);
        header("Location: " . BASE_URL . "/daftar_dudukan?show_archived=true"); // Redirect to refresh the page, staying on archived view
        exit;
    } catch (PDOException $e) {
        echo "<p>Error unarchiving dudukan: " . htmlspecialchars($e->getMessage()) . "</p>";
    }
}
?>
<style>
...
<h1 class="text-xl sm:text-2xl font-bold mb-6 text-gray-800 flex flex-col sm:flex-row sm:items-center">
    <span class="mr-4">daftar dudukan</span>
    <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
    <div class="flex flex-col sm:flex-row sm:inline-flex mt-2 sm:mt-0 sm:ml-4 space-y-2 sm:space-y-0 sm:space-x-2">
        <a href="bikin_dudukan" class="px-4 py-2 bg-blue-600 text-white font-bold hover:bg-blue-700 transition duration-150 ease-in-out rounded-md">+ Dudukan</a>
        <?php if (isset($_GET['show_archived']) && $_GET['show_archived'] == 'true'): ?>
            <a href="daftar_dudukan" class="sm:ml-2 px-4 py-2 bg-gray-600 text-white font-bold hover:bg-gray-700 transition duration-150 ease-in-out rounded-md">Active Dudukan</a>
        <?php else: ?>
            <a href="daftar_dudukan?show_archived=true" class="sm:ml-2 px-4 py-2 bg-gray-600 text-white font-bold hover:bg-gray-700 transition duration-150 ease-in-out rounded-md">Archived Dudukan</a>
        <?php endif; ?>
    </div>
    <?php endif; ?>
</h1>

<form action="daftar_dudukan" method="get" class="mb-4 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
    <input type="text" name="search" placeholder="cari data dudukan" value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>" class="p-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-md w-full sm:w-auto">
...
<?php
try {
    $sql = "SELECT id, jenis, is_archived FROM dudukan";
    $conditions = [];
    $params = [];
    ...
    if ($dudukan_data) {
        ...
        foreach ($dudukan_data as $dudukan) {
            echo "<tr>";
            ...
            if ($is_admin) {
                ...
                echo "<a href=\"edit_dudukan?id=". htmlspecialchars($dudukan['id']) . "\" class=\"text-indigo-600 hover:text-indigo-900\">Edit</a>";
                if (isset($_GET['show_archived']) && $_GET['show_archived'] == 'true') {
                    echo "<a href=\"daftar_dudukan?unarchive_id=". htmlspecialchars($dudukan['id']) . "\" onclick=\"return confirm('Are you sure you want to unarchive this dudukan?');\" class=\"text-green-600 hover:text-green-900\">Unarchive</a>";
                } else {
                    echo "<a href=\"daftar_dudukan?archive_id=". htmlspecialchars($dudukan['id']) . "\" onclick=\"return confirm('Are you sure you want to archive this dudukan?');\" class=\"text-red-600 hover:text-red-900\">Archive</a>";
                }
                echo "</td>";
            }
            echo "</tr>";
        ...
        echo "<p class=\"mt-4 text-gray-600\">No dudukan found in the database. Click +Dudukan to create a new one.</p>";
    }
} catch (PDOException $e) {
