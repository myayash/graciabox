...
if (isset($_GET['archive_id']) && !empty($_GET['archive_id'])) {
    ...
    try {
        ...
        header("Location: " . BASE_URL . "/daftar_karyawan_sales"); // Redirect to refresh the page
        exit;
...
if (isset($_GET['unarchive_id']) && !empty($_GET['unarchive_id'])) {
    ...
    try {
        ...
        header("Location: " . BASE_URL . "/daftar_karyawan_sales?show_archived=true"); // Redirect to refresh the page, staying on archived view
        exit;
    } catch (PDOException $e) {
...
<h1 class="text-xl sm:text-2xl font-bold mb-6 text-gray-800 flex flex-col sm:flex-row sm:items-center">
    <span class="mr-4">daftar karyawan sales</span>
    <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
    <div class="flex flex-col sm:flex-row sm:inline-flex mt-2 sm:mt-0 sm:ml-4 space-y-2 sm:space-y-0 sm:space-x-2">
        <a href="bikin_karyawan_sales" class="px-4 py-2 bg-blue-600 text-white font-bold hover:bg-blue-700 transition duration-150 ease-in-out rounded-md">+ Karyawan Sales</a>
        <?php if (isset($_GET['show_archived']) && $_GET['show_archived'] == 'true'): ?>
            <a href="daftar_karyawan_sales" class="sm:ml-2 px-4 py-2 bg-gray-600 text-white font-bold hover:bg-gray-700 transition duration-150 ease-in-out rounded-md">Active Karyawan Sales</a>
        <?php else: ?>
            <a href="daftar_karyawan_sales?show_archived=true" class="sm:ml-2 px-4 py-2 bg-gray-600 text-white font-bold hover:bg-gray-700 transition duration-150 ease-in-out rounded-md">Archived Karyawan Sales</a>
        <?php endif; ?>
    </div>
    <?php endif; ?>
</h1>

<form action="daftar_karyawan_sales" method="get" class="mb-4 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
    <input type="text" name="search" placeholder="cari data karyawan sales" value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>" class="p-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-md w-full sm:w-auto">
...
try {
    ...
    if (isset($_GET['search']) && $_GET['search'] !== '') {
        ...
        $searchable_columns = ['id', 'nama', 'dibuat'];
    ...
    if ($karyawan_sales_items) {
        ...
        foreach ($karyawan_sales_items as $karyawan_sales) {
            ...
            if ($is_admin) {
                ...
                echo "<a href=\"edit_karyawan_sales?id=" . htmlspecialchars($karyawan_sales['id']) . "\" class=\"text-indigo-600 hover:text-indigo-900\">Edit</a>";
                if (isset($_GET['show_archived']) && $_GET['show_archived'] == 'true') {
                    echo "<a href=\"daftar_karyawan_sales?unarchive_id=" . htmlspecialchars($karyawan_sales['id']) . "\" onclick=\"return confirm('Are you sure you want to unarchive this karyawan sales?');\" class=\"text-green-600 hover:text-green-900\">Unarchive</a>";
                } else {
                    echo "<a href=\"daftar_karyawan_sales?archive_id=" . htmlspecialchars($karyawan_sales['id']) . "\" onclick=\"return confirm('Are you sure you want to archive this karyawan sales?');\" class=\"text-red-600 hover:text-red-900\">Archive</a>";
                }
            }
            echo "</td>";
            echo "</tr>";
