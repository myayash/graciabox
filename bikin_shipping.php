...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Store the first form's data in the session
...
<head>
    ...
    <title>bikin form order - shipping</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 text-gray-900 pt-24 px-8 pb-8 font-mono">

    <?php include 'navbar.php'; ?>
    <h1 class="text-2xl font-bold mb-6 text-gray-800">bikin form order - shipping</h1>
    ...
    <form action="process_order.php" method="post" class="bg-white p-8 shadow-lg">
        ...
        <div class="mb-4">
            ...
            <input type="date" name="tanggal_kirim" id="tanggal_kirim" class="appearance-none bg-white border border-gray-300 w-full py-2 px-3 text-gray-800 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out" required>
        </div>

        <div class="mb-4">
            <label for="jam_kirim" class="block text-gray-800 text-sm font-semibold mb-2">Jam Kirim:</label>
            <input type="time" name="jam_kirim" id="jam_kirim" class="appearance-none bg-white border border-gray-300 w-full py-2 px-3 text-gray-800 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="dikirim_dari" id="dikirim_dari" class="appearance-none bg-white border border-gray-300 w-full py-2 px-3 text-gray-800 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out" required>
                ...
                <?php foreach ($alamat_pengirim as $alamat): ?>
                    <option value="<?= $alamat['lokasi'] ?>"><?= $alamat['lokasi'] ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="tujuan_kirim" id="tujuan_kirim" class="appearance-none bg-white border border-gray-300 w-full py-2 px-3 text-gray-800 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out" required>
        </div>
        ...
        <div class="flex items-center justify-start space-x-4">
            ...
            <a href="bikin_fo.php" class="inline-block align-baseline font-semibold text-sm text-blue-600 hover:text-blue-800">
                Back
            </a>
        </div>
    </form>
